...
void php_driver_timeuuid_init(INTERNAL_FUNCTION_PARAMETERS) {
  ...
  if (getThis() &&
      instanceof_function(Z_OBJCE_P(getThis()), php_driver_timeuuid_ce)) {
      ...
      case IS_STRING:
        if (cass_uuid_from_string(Z_STRVAL_P(param), &self->uuid) != CASS_OK) {
          zend_throw_exception_ex(php_driver_invalid_argument_exception_ce, 0,
                                  "Invalid UUID: '%.*s'", Z_STRLEN_P(param),
                                  Z_STRVAL_P(param));
          return;
        }
...
PHP_METHOD(Timeuuid, toDateTime) {
  ...
  str_len =
      spprintf(&str, 0, "@%ld", (long)(cass_uuid_timestamp(self->uuid) / 1000));
...
static zend_function_entry php_driver_timeuuid_methods[] = {
    PHP_ME(Timeuuid, __construct, arginfo__construct,
...
static HashTable *php_driver_timeuuid_properties(
#if PHP_MAJOR_VERSION >= 8
    zend_object *object
#else
    zval *object
  ...
  PHP5TO7_ZEND_HASH_UPDATE(props, "uuid", sizeof("uuid"),
                           PHP5TO7_ZVAL_MAYBE_P(uuid), sizeof(zval));

  PHP5TO7_ZVAL_MAYBE_MAKE(version);
  ZVAL_LONG(PHP5TO7_ZVAL_MAYBE_P(version), (long)cass_uuid_version(self->uuid));
  PHP5TO7_ZEND_HASH_UPDATE(props, "version", sizeof("version"),
                           PHP5TO7_ZVAL_MAYBE_P(version), sizeof(zval));
...
static int php_driver_timeuuid_compare(zval *obj1, zval *obj2) {
  ...
  if (uuid1->uuid.clock_seq_and_node != uuid2->uuid.clock_seq_and_node)
    return uuid1->uuid.clock_seq_and_node < uuid2->uuid.clock_seq_and_node ? -1
                                                                           : 1;
...
static unsigned php_driver_timeuuid_hash_value(zval *obj) {
  ...
  return php_driver_combine_hash(
      php_driver_bigint_hash(self->uuid.time_and_version),
...
void php_driver_define_Timeuuid() {
  ...
  INIT_CLASS_ENTRY(ce, PHP_DRIVER_NAMESPACE "\\Timeuuid",
                   php_driver_timeuuid_methods);
  ...
  php_driver_timeuuid_handlers.std.get_properties =
...
#else
  php_driver_timeuuid_handlers.std.compare_objects =
      php_driver_timeuuid_compare;
#endif
